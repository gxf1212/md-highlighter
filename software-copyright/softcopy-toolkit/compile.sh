...
compile_single() {
    ...
    xelatex -synctex=1 -interaction=nonstopmode "$tex_file" > /tmp/xelatex_pass1.log 2>&1
    local pass1_status=$?

    # 如果有.bib文件，运行BibTeX
    if [ -f "${base_name}.bib" ]; then
        print_info "运行BibTeX处理参考文献..."
        bibtex "${base_name}" > /tmp/bibtex.log 2>&1 || {
    ...
    xelatex -synctex=1 -interaction=nonstopmode "$tex_file" > /tmp/xelatex_pass2.log 2>&1
    local pass2_status=$?

    # 第三次编译 - 确保所有引用正确
    print_info "第三次XeLaTeX编译..."
    xelatex -synctex=1 -interaction=nonstopmode "$tex_file" > /tmp/xelatex_pass3.log 2>&1
    local pass3_status=$?

    # 检查PDF是否生成
    if [ -f "${base_name}.pdf" ]; then
        local pdf_size=$(ls -lh "${base_name}.pdf" | awk '{print $5}')
        local pdf_pages=$(pdfinfo "${base_name}.pdf" 2>/dev/null | grep "Pages:" | awk '{print $2}')
        print_success "成功生成 ${base_name}.pdf (${pdf_pages} 页, ${pdf_size})"

        # 显示警告（如果有）
        if [ $pass1_status -ne 0 ] || [ $pass2_status -ne 0 ] || [ $pass3_status -ne 0 ]; then
            print_warning "编译过程中有警告，但PDF已成功生成"
            print_info "查看日志: /tmp/xelatex_pass*.log"
        fi
        return 0
    else
...
main() {
    ...
    # 检查输出文件
    if [ -f "${base_name}.pdf" ]; then
        print_success "编译完成！生成文件：${base_name}.pdf"
        print_info "PDF文件大小：$(du -h ${base_name}.pdf | cut -f1)"

        # 可选：打开PDF文件（使用 --open 参数时才询问）
        if [[ "${OPEN_PDF:-}" == "true" ]] && command -v evince &> /dev/null; then
...
show_help() {
    ...
    echo ""
    echo "用法："
    echo "  ./compile.sh                    - 编译document.tex（默认）"
    echo "  ./compile.sh <file.tex>         - 编译指定的.tex文件"
    echo "  ./compile.sh --open             - 编译后可选择打开PDF"
    echo "  ./compile.sh clean              - 清理临时文件"
    echo "  ./compile.sh help               - 显示此帮助信息"
    echo ""
    echo "示例："
    echo "  ./compile.sh source-code.tex    - 编译源代码文档"
    echo "  ./compile.sh document.tex       - 编译使用说明文档"
    echo ""
    echo "环境变量："
    echo "  OPEN_PDF=true                   - 编译后询问是否打开PDF"
    echo ""
    echo "环境要求："
    echo "  - Ubuntu系统"
